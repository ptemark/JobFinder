...
### T32 — Wire Sheets sync into the status endpoint + config  **[P1]**  `[x] Complete`
- **Depends on:** T31, T30
...
|---|------|------|-------|-------|
| 33 | 2026-06-09 | T32 Wire Sheets sync into status endpoint + config | src/jobfinder/settings.py, .env.example, .gitignore, src/jobfinder/web/api.py, tests/test_api.py, tests/test_settings.py, spec/tasks.md | **Final M7 task — project T01–T33 now complete.** Closes the Sheets pair: T31 shipped the `sheets.sync_applied` client reading `settings.sheets_enabled`/`google_sheets_credentials`/`job_tracker_sheet_id`/`job_tracker_sheet_gid`; this task **adds those settings** and **wires the sync into the status endpoint** (LLD §11.4/§16/§9.1). **settings.py:** four optional M7 fields on `Settings` (env aliases `GOOGLE_SHEETS_CREDENTIALS` `Path\|None`, `JOB_TRACKER_SHEET_ID` `str\|None`, `JOB_TRACKER_SHEET_GID` `int\|None`, all default None) + a `sheets_enabled` property true **only when both the credentials path AND the sheet id are set** — the Adzuna both-keys gate (spec §15: absent → marking "applied" still works locally, sync skips cleanly). Added `env_ignore_empty=True` to `model_config` so a copied-but-blank `JOB_TRACKER_SHEET_GID=` in `.env` parses as unset rather than failing `"" → int` validation (the int gid is the only field where an empty env string isn't already valid; the change is behaviour-equivalent for the existing `str\|None` Adzuna/JSearch keys — empty → None vs "" both read as disabled). **.env.example:** documented the three M7 keys (LLD §11.3) with the service-account/sheet-share setup note. **.gitignore:** added `config/google-service-account.json` (the key file, never committed — Cost & Safety §4 / LLD §11.3). **web/api.py:** `POST /api/jobs/{id}/status` now reuses the row it already fetches for the 404 check, and **after** the authoritative `set_status`, calls a new `_sync_applied_best_effort(settings, state, row)` helper — **only when `state == Status.APPLIED`** (other states never touch Sheets), mapping the `SyncResult` to `StatusResponse.sheet_synced` (`"appended"` → True, else False) and **logging** a `result.status == "error"` via the module `logger` — never 500s the request (the §16.1 bulkhead already caught the network failure inside `sync_applied`). A frozen `_AppliedJob` dataclass (`from_row`) adapts the `sqlite3.Row` (subscript) to the `sheets.AppliedJob` Protocol (attribute access on `company`/`title`/`url`). The status write stays the source of truth regardless of the Sheets outcome (HLD §3.7/D14). **Tests:** `tests/test_api.py` — `test_status_applied_invokes_sheets_sync_once` (patches `jobfinder.web.api.sync_applied`, asserts one call carrying Acme/title/url + `sheet_synced=True` on `"appended"`); `test_status_applied_sheets_error_still_returns_200` (patched `"error"` → 200, `sheet_synced=False`, status still persisted); `test_status_non_applied_never_calls_sheets` (dismissed/interested/new → zero sync calls, `sheet_synced=False`). The existing `test_applied_hidden_…` already covers the unconfigured path (real `sync_applied`, `sheets_enabled` False → `{"ok":True,"sheet_synced":False}`, zero network). `tests/test_settings.py` — `sheets_enabled` disabled-when-unconfigured, true-only-with-both, and blank-gid-env → None. All Sheets-sync tests are **fully offline** (patched sync or unconfigured skip; no `MockTransport` needed at the API layer — T31 already covers the transport). No new deps (google-auth already added in T31; Sheets REST reuses httpx). CI green (283 tests, ruff + format clean). **M7 complete — the stricter remote filter (T29), Applied tab + hide (T30/T33), Sheets sync (T31/T32) and UI restyle (T33) all shipped; the whole project T01–T33 is done.** |
