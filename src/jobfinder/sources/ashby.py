"""Ashby Posting API adapter (LLD §3.5).

Fetches the public job board for each configured Ashby board token and returns
the postings as :class:`RawPosting`s, **dropping anything older than
...
Like Greenhouse, Ashby hands back ``{"jobs": [...]}`` in one response (no
pagination). Every field access is guarded: a malformed posting is skipped and
counted, and one board failing (HTTP error, bad JSON, unexpected shape) is
isolated so the other configured boards still return (per-board bulkhead, RALPH
No-Shortcut rules). The verbatim provider object — including the optional
``compensation`` summary (LLD §3.5) — is preserved on each ``RawPosting`` for
...
from jobfinder.normalize import ashby_posted_value, parse_date
...
# Public posting API: full job board for a board token, no auth (LLD §3.5).
ASHBY_BOARD_URL = "https://api.ashbyhq.com/posting-api/job-board/{token}"
# ``includeCompensation=true`` adds the salary summary to each posting (LLD §3.5).
ASHBY_PARAMS = {"includeCompensation": "true"}


class AshbySource:
    """Ashby board adapter implementing the :class:`Source` protocol."""
    ...
    def fetch(self, *, max_age_days: int, throttle_s: float) -> SourceResult:
        """Fetch every configured board, dropping stale postings (LLD §3.5).
    ...
    def _fetch_board(
        ...
        *,
        ...
        url = ASHBY_BOARD_URL.format(token=company.token)
        try:
            payload = self._client.get_json(url, params=ASHBY_PARAMS)
    ...
    def _consume_job(
        ...
        *,
        ...
        # Ashby exposes both ``publishedAt`` and ``updatedAt`` (ISO8601); prefer
        # the publish date, falling back to the update date (LLD §3.5).
        posted_at = parse_date(ashby_posted_value(job), self.name)
        ...
        if posted_at is not None and (now - posted_at).days > max_age_days:
            return  # stale: excluded before normalize/embed (LLD §3.5)
        ...
        result.raw.append(
            RawPosting(
                ...
                payload=job,
                # company_name is not in the Ashby payload; the configured name
                # (or token) is the documented fallback for normalize (LLD §3.5).
                company_hint=company.name or company.token,
            )
        )


def build_ashby_source(settings: Settings) -> AshbySource:
    """Construct the adapter from settings: its boards + the shared HTTP client."""
    companies = load_companies(settings.config_dir / "companies.yaml").ashby
    return AshbySource(companies=companies, client=get_default_client())


register_source(SOURCE_NAME, build_ashby_source)
